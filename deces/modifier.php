...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($patiente_id) || empty($date_deces) || empty($cause_deces)) {
        ...
        try {
            ...
            $db->query("UPDATE deces SET patiente_id=?, date_deces=?, cause_deces=?, age_deces=?, lieu_deces=?, observations=? WHERE id=?",
                [$patiente_id, $date_time_deces, $cause_deces, $age_deces, $lieu_deces, $observations, $deces_id]);
            
            header('Location: voir.php?id=' . $deces_id . '&success=1');
            exit;
        } catch (Exception $e) {
...
<head>
    ...
    <title>Modifier le Décès - Clinique Obstétrique</title>
    ...
    <div class="flex">
        ...
        <main class="flex-1 p-8">
            ...
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-gray-900 mb-2">Modifier le Décès</h2>
                ...
                <form method="POST" class="space-y-6">
                    <div>
                        ...
                        <select name="patiente_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                            ...
                            <?php foreach ($patientes as $pat): ?>
                                <option value="<?php echo $pat['id']; ?>" <?php echo ($deces['patiente_id'] == $pat['id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="date_deces" value="<?php echo date('Y-m-d', strtotime($deces['date_deces'])); ?>" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Heure du décès</label>
                            <input type="time" name="heure_deces" value="<?php echo date('H:i', strtotime($deces['date_deces'])); ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Cause du décès *</label>
                        <input type="text" name="cause_deces" value="<?php echo htmlspecialchars($deces['cause_deces']); ?>" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Âge au décès (heures)</label>
                            <input type="number" name="age_deces" value="<?php echo htmlspecialchars($deces['age_deces'] ?? ''); ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Lieu du décès</label>
                            <input type="text" name="lieu_deces" value="<?php echo htmlspecialchars($deces['lieu_deces'] ?? ''); ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                        </div>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Observations</label>
                        <textarea name="observations" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"><?php echo htmlspecialchars($deces['observations'] ?? ''); ?></textarea>
                    </div>
