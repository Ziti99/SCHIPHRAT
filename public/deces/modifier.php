...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($date_deces) || empty($cause_deces) || empty($medecin_id)) {
        $error = 'Veuillez remplir tous les champs obligatoires (date de décès, cause et médecin).';
    } else {
        try {
            $db->query("
                UPDATE deces SET
                    date_deces = ?, cause_deces = ?, age_deces = ?, lieu_deces = ?, 
                    medecin_id = ?, observations = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
            ", [
            ...
            $success = 'Décès modifié avec succès !';
            
            // Recharger les données du décès
            $deces = $db->fetch("
                SELECT d.*, 
                       p.nom, p.prenom, p.date_naissance
                FROM deces d
...
<head>
    ...
    <title>Modifier Décès - Clinique Obstétrique</title>
    ...
    <div class="flex">
        ...
        <div class="flex-1">
                    ...
                    <div class="flex justify-between h-16">
                        <div class="flex items-center">
                            ...
                            <div class="flex-shrink-0 flex items-center">
                                ...
                                <span class="text-xl font-bold text-gray-900">Modifier Décès</span>
                            </div>
                        </div>
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <!-- En-tête -->
                <div class="mb-8">
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Modifier Décès</h1>
                    <p class="text-gray-600">
                        Décès de <?php echo htmlspecialchars($deces['prenom'] . ' ' . $deces['nom']); ?>
                    ...
                    <form method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="datetime-local" name="date_deces" required
                                       value="<?php echo date('Y-m-d\TH:i', strtotime($deces['date_deces'])); ?>"
                            ...
                            <div>
                                ...
                                <select name="medecin_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                                    ...
                                    <?php foreach ($medecins as $medecin): ?>
                                        <option value="<?php echo $medecin['id']; ?>" <?php echo $deces['medecin_id'] == $medecin['id'] ? 'selected' : ''; ?>>
                        ...
                        <div>
                            ...
                            <textarea name="cause_deces" rows="3" required
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                                      placeholder="Décrivez la cause du décès..."><?php echo htmlspecialchars($deces['cause_deces']); ?></textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="age_deces" min="0" max="120"
                                       value="<?php echo htmlspecialchars($deces['age_deces'] ?? ''); ?>"
                            ...
                            <div>
                                ...
                                <input type="text" name="lieu_deces"
                                       value="<?php echo htmlspecialchars($deces['lieu_deces'] ?? ''); ?>"
                        ...
                        <div class="border-t border-gray-200 pt-6">
                            ...
                            <div>
                                ...
                                <textarea name="observations" rows="4"
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                                          placeholder="Observations complémentaires sur les circonstances du décès..."><?php echo htmlspecialchars($deces['observations'] ?? ''); ?></textarea>
                            </div>
                        </div>
